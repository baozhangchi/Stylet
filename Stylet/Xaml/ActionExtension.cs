...
namespace Stylet.Xaml
{
	...
	public class ActionExtension : MarkupExtension
	{
		...
		private object HandleDependencyObject(IServiceProvider serviceProvider, IProvideValueTarget valueService, DependencyObject targetObject)
		{
			switch (valueService.TargetProperty)
			{
				case DependencyProperty dependencyProperty when dependencyProperty.PropertyType == typeof(ICommand):
					...
					return this.CreateCommandAction(serviceProvider, targetObject, dependencyProperty);
		...
		private ICommand CreateCommandAction(IServiceProvider serviceProvider, DependencyObject targetObject, DependencyProperty dependencyProperty)
		{
			if (targetObject is UIElement uiElement)
			{
				return new RoutedUICommandAction(uiElement, Target, Method, CommandNullTargetBehaviour, CommandActionNotFoundBehaviour, dependencyProperty);
			}
			if (this.Target == null)
			{
